...
def render(data: dict, sources: dict) -> FilterState:
    ...
    with st.sidebar:
        ...
        # ⚠ 라벨에 '(V)' 만 적지 않는다. COMP_PATTERN 은 `(V)` 접두 외에 '무료'·
        # '(무)' 표기도 잡고, 실제 물량은 '무료' 쪽이 13배 크다(30,560 vs
        # 414,912개). 라벨이 (V) 만 말하면 토글을 켠 사람이 예상의 10배가
        # 빠지는 것을 보게 된다 — 실측 합계는 전체 수량의 5.9% 다.
        exclude_comp = st.toggle(
            "컴프 · 무상 제공 상품 제외", value=False, key="flt_comp",
            help="'(V)산삼배양근진' 같은 (V) 접두 상품과 '18)헛개차(무료)' 같은 "
                 "무료 표기 상품을 함께 제외합니다. 합쳐서 전체 수량의 약 5.9%이며, "
                 "'무료' 표기 쪽이 (V) 접두보다 13배 큽니다.",
        )
