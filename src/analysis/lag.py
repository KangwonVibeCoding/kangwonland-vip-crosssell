...
ARS 유입(D)과 채널 판매량(D+k)의 상관을 k=0..3 으로 훑는다.

⚠ **결론이 창 길이에 따라 갈린다. 이 모듈을 인용할 때는 창을 반드시 함께 적는다.**

    구간 A (2024-12, 31일)   카지노 D+0 0.796 · 룸 D+0 0.733 · 특산품 **D+1 0.467**
    구간 B (2023~24, 731일)  카지노 D+0 0.761 · 룸 D+0 0.779 · 특산품 **D+0 0.313**

2026-08-11 에 ARS 전처리 통합본이 들어와 조인 가능일이 31일 → 731일이 되면서
**"특산품 D+1" 은 31일 창의 산물로 판명됐다.** 두 해가 각각 독립적으로 D+0 을
가리킨다(2023: 0.490 vs 0.299 · 2024: 0.422 vs 0.261). 원래도 `(D+1 r) − (D+0 r)`
의 95% CI 가 0 을 포함해 근거가 약했고, 보강 근거였던 '토요일 0.92'(토요일 표본
4일)도 2년 창에서 1.20 이라 재현되지 않았다.

**다만 요일 피크가 하루 어긋나는 현상 자체는 731일에서도 참이다** — 유입은
토요일(1.38)에, 특산품은 일요일(1.36)에 정점이다. 주말 내내 둘 다 높아서 당일
상관이 더 강할 뿐이다. "체크아웃 선물" 해석은 이 피크 어긋남으로 말하고,
래그 상관으로는 말하지 않는다.

⚠ 위 상관은 **요일을 통제하지 않은 값**이다. 731일 창에서 요일 더미를 빼면
카지노 0.801 → 0.617, 룸서비스 0.765 → 0.596 이고 세 채널 모두 p ≤ 0.001 이다
(`stats.confound_table`). `prescriptions()` 는 집행 순서를 래그가 아니라
편상관으로 매긴다. 31일 창에서는 그 통제가 순서를 뒤집었지만(카지노 0.370 <
룸서비스 0.558) 731일에서는 카지노가 1위를 되찾는다 — 이것도 창의 문제였다.
"""
