...
# ── 분석 구간 (실데이터 기간 불일치를 정직하게 다루기 위한 설계) ──────
#   A: 2024-12 정밀 검증 창 — 최초 분석이 여기서 나왔고 회귀 기준값의 원점이다
#   B: 2023~2024 판매 전 기간 실측 조인 (731일) → **지금의 기본 창**
#
# 2026-08-11 에 있던 구간 C(2025~2026 상반기, ARS만)는 **제거했다.** 그 구간의
# 판매 데이터를 확보할 수 없어 탭 2·3·4 가 항상 안내 화면으로 뜨는데, 심사자가
# 그것을 고장으로 읽는다(USER_GUIDE 에 "십중팔구 구간이 최신 유입입니다"라는
# 문제해결 항목을 따로 둬야 했을 정도다). ARS 원본은 2026-06 까지 그대로 있으므로
# 판매가 확보되면 이 딕셔너리에 항목을 되돌리는 것만으로 복구된다.
#
# 2026-08-11 에 ARS 전처리 통합본(2023-01~2026-06, 1,247일)이 들어오면서
# 구간 B 가 "요일 브릿지"(판매만 있는 구간의 프록시)에서 **실측 조인**으로
# 승격됐다. 조인 가능일 31일 → 731일. 구간 A 는 지우지 않고 남긴다 —
# 회귀 기준값 다수가 그 창의 실측이고, 창 길이에 따라 결론이 갈리는 지표
# (특산품 래그·토요일 지수)를 대조하려면 두 창이 다 있어야 하기 때문이다.
#
# ⚠ 통합본에는 `ARS접수자`·`모바일접수자` 가 없다. 2024-12 원본 파일이
#   그 31일분을 보완하므로 **두 파일을 함께 두어야 한다** (로더가 날짜로
#   중복 제거하며, 파일명 정렬상 원본이 뒤에 와서 이긴다).
PERIOD_A = "A"
PERIOD_B = "B"
...
PERIODS: dict[str, dict] = {
    ...
    PERIOD_B: {
        "label": "판매 전 기간 · 2023~2024 (ARS 실측 조인 731일)",
        "badge": "실측 조인",
...
# 기본 창을 731일 실측 조인으로 둔다 — 같은 지표가 31일 창에서는 표본 부족으로
# 다른 답을 내기 때문이다(특산품 래그 D+1 vs D+0). 넓은 창이 기본이어야 한다.
DEFAULT_PERIOD = PERIOD_B
...
COLUMN_MAP: dict[str, str] = {
    ...
    # ARS 예약현황
    "내국인총접수자": "recv_total",
    # 전처리 통합본(2023-01~2026-06)은 접두어 없이 짧은 이름으로 준다.
    # 총당첨자 컬럼이 없고 ARS·모바일 당첨자만 오므로 normalize_ars 가 합으로
    # 복원한다 — 2024-12 원본 31일과 합계가 정확히 일치함을 확인했다.
    "총접수자": "recv_total",
    "입장권구매건수": "tickets",
    "입장권구매율": "buy_rate",
    "ars접수자": "recv_ars",
