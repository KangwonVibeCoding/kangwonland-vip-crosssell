"""AI VIP 큐레이션 — 세그먼트 페르소나 × 기준일 → 상품 추천 3슬롯.

기존 분석 함수(`inflow`·`crosssell`·`lag`·`stats`·`vip`·`scoring`)만 조합한다.
DataFrame in / dict out 순수 모듈이라 **streamlit 을 import 하지 않는다** — 탭
뷰(`src/views/curation.py`)와 CLI(`scripts/prototype_curation.py`)가 같이 쓴다.
...
20일 스윕에서 고유 추천 조합이 14개 → 2개로 주저앉는다(실측).
"""
...
from src.analysis import crosssell, lag, scoring, stats, vip
...
def intent_summary(as_of: pd.Timestamp, day: pd.Series, persona: dict,
    ...
    return (
        ...
        f"({persona['gender']}성 내 {share_in_gender:.1%})"
        f" — 원본이 누적 고객수라 구성비로만 인용한다. "
...
def curate(ars: pd.DataFrame, sales: pd.DataFrame, demo: pd.DataFrame,
           persona_id: str, as_of: pd.Timestamp) -> dict:
    """기준일 하루치 추천 3슬롯 + 진단 정보.

    반환의 `payload` 가 확정 스키마(customer_id / today_intent_summary /
    recommendations×3)이고, 나머지 키는 화면·CLI 가 근거를 펼쳐 보이기 위한
    진단값이다. 예외를 밖으로 던지되 `SystemExit` 은 쓰지 않는다 — 분석 계층이
    프로세스를 죽이면 탭 단위 예외 격리가 무의미해진다.
    """
    ...
    row = inflow.loc[inflow["date"] == as_of] if not inflow.empty else inflow
    if row.empty:
        span = (f"{inflow['date'].min():%Y-%m-%d} ~ {inflow['date'].max():%Y-%m-%d}"
                if not inflow.empty else "없음")
        raise ValueError(f"기준일 {as_of:%Y-%m-%d} 의 유입 데이터가 없습니다. 가용 구간: {span}")
    day = row.iloc[0]
    grade = str(day["grade"])
    if sales.loc[sales["date"] == as_of].empty:
        warnings.append(f"기준일 {as_of:%Y-%m-%d} 에 판매 데이터가 없습니다 — 유입 신호로만 추천합니다.")
    ...
    if not slots:
        raise ValueError("후보가 모두 탈락했습니다 — 기간·필터를 넓혀 주세요.")
    ...
    return {"payload": payload, "persona": persona, "persona_id": persona_id,
            "day": day, "grade": grade, "funnel": funnel, "scored": scored,
            "gate": gate, "ch_sig": ch_sig, "seg_pref": seg_pref, "seg": seg,
            "slots": slots, "share": share, "share_gender": share_gender,
            "warnings": warnings}
