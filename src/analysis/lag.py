...
**익일 오전엔 특산품 쿠폰**. 요일 인덱스(특산품 일요일 1.63)와도 일관된다.

⚠ 래그 상관의 표본은 구간 A 31일뿐이고 `(D+1 r) − (D+0 r)` 의 95% CI 가 0 을
포함한다. 보강 근거인 요일 프로파일도 **절반만 창을 견딘다** — 일요일 피크는
2023·2024 각각 1.36 으로 재현되지만, 구간 A 의 '토요일 0.92'(토요일 표본 4일)는
2년 창에서 1.20 이라 재현되지 않는다. 인용할 때 창 길이를 함께 적을 것.
