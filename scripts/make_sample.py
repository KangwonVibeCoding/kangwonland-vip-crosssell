...
무엇을 담는가:
  **연간 전체를 정규화된 parquet 으로** 담는다. 기간을 자르지 않는 이유는 월 인덱스
  (룸서비스 1월 1.21 · 특산품 9월 1.43 — 2년 평균)가 계절성 분석의 핵심 근거이기
  때문이다. CSV 12.4MB 를 parquet 으로 저장하면 문자열 사전 인코딩 덕에 크게 줄어든다.

무엇을 담지 않는가:
  실데이터로 확보되지 않은 데이터셋은 내보내지 않는다. 내장 폴백 데이터를
  'SAMPLE' 로 커밋하면 모킹을 실데이터로 위장하는 셈이 된다. (성별·연령이 오래
  이 자리에 있었으나 2026-08-08 에 실데이터로 들어와 지금은 전부 내보낸다.)
