...
<never>
1. `src/data`, `src/analysis` 에 `import streamlit` — 151개 테스트가 전부 깨진다
...
수치를 문서에 적을 때는 **어느 구간(A/B)·어느 창 길이의 값인지 함께 적는다.**
...
⚠ **`src/views/` 는 `scripts/` 를 import 하지 않는다.** 스크립트와 화면이 같은
로직을 써야 하면 로직을 `src/analysis/` 로 올리고 양쪽이 그것을 부른다 —
`curation.py` 가 그 사례다(탭5 와 `scripts/prototype_curation.py` 가 공유).
스크립트에 로직을 두고 뷰가 끌어다 쓰면 화면과 CLI 가 조용히 다른 답을 낸다.

`src/views/` 를 절대 `src/pages/` 로 바꾸지 말 것 — Streamlit 자동 멀티페이지와 충돌한다.

### 시간축 2구간 설계 (이 프로젝트의 핵심 구조)
...
| **B** | **2023~2024 (731일)** | **ARS + 판매** | **실측 조인 — 상관·래그·lift 의 본 창 (기본값)** | `실측 조인` |

**구간 C(2025~2026 상반기, ARS만)는 2026-08-11 에 제거했다.** 그 구간 판매 데이터를
확보할 수 없어 탭 2~5 가 항상 안내 화면으로 떴고, 심사자가 그것을 고장으로 읽는다
(USER_GUIDE 에 "탭이 비어 있다 = 십중팔구 구간이 최신 유입입니다"라는 문제해결
항목을 따로 둬야 했을 정도다). **ARS 원본은 2026-06 까지 그대로 있으므로** 판매가
확보되면 `PERIODS` 에 항목을 되돌리는 것만으로 복구된다. `test_stats.py` 의
`period_c` 픽스처(총접수자 없는 ARS 폴백)는 그대로 둔다 — 그런 파일이 또 들어올 수
있고, UI 구간과 무관하게 코드 경로를 지키는 테스트다.

⚠ **사이드바 기본 선택은 `S.DEFAULT_PERIOD` 를 읽어야 한다.** `st.radio(index=0)`
으로 박아두면 딕셔너리 순서상 항상 구간 A(31일 창)가 잡히고, 첫 화면이 **철회된
서사**(특산품 D+1 · 편상관 룸서비스 1위)를 헤드라인으로 띄운다. 2026-08-11 에 이
버그를 고쳤다.

**2026-08-11 에 ARS 전처리 통합본(2023-01~2026-06, 1,247일)이 들어오면서 구간 B 가
프록시(CAI·요일 브릿지)에서 실측 조인으로 승격됐다.** 조인 가능일 31일 → 731일.
`DEFAULT_PERIOD` 도 A → B 로 옮겼다. 구간 A 는 지우지 않는다 — **같은 코드가 창에
따라 다른 답을 내는 사례**(특산품 래그 D+1 vs D+0)를 대조하려면 두 창이 다 필요하다.
...
| 파일 | 담는 것 | 왜 필요한가 |
|---|---|---|
| `ars_20230101_20260630.csv` | 2023-01~2026-06 **1,247일** (2025-11 결측) | 실측 조인의 본체. 총접수자를 전 구간에 채운다 |
| `ars_20241201_20241231.csv` | 2024-12 31일 (9컬럼) | **ARS·모바일 *접수자* 분해**를 보완. 통합본에는 당첨자 분해만 있다 |

로더는 날짜로 중복을 제거하며(`keep="last"`), 파일명 정렬상 `..._20241201_...` 이
뒤에 와서 그 31일은 원본이 이긴다 — 컬럼이 더 많은 쪽이 남는다. **두 파일을 함께
두어야 한다.** 2025-11 은 이상치라 통합본 단계에서 제외됐다(의도된 결측).

`_superseded_ars_merged.csv` 는 옛 2026 전처리본이다. 통합본이 같은 181일을 값
오차 0 으로 포함하면서 총접수자까지 주므로 glob(`ars_*.csv`) 밖으로 빼 두었다.
지운 게 아니라 이름만 바꿨다 — 필요하면 되돌릴 수 있다.

`data/raw/` 는 gitignore 되므로 커밋본은 `data/sample/ars.parquet` 안에 있다.
2024-12 원본이 사라졌으면 이렇게 되살린다:
...
- **편상관** (`stats.confound_table`) — 요일 더미 6개로 양변을 회귀한 잔차끼리의 상관.
  ...
  (731일: 카지노 0.617 · 룸서비스 0.596 · 특산품 0.214, 셋 다 p ≤ 0.001).
  ⚠ 31일 창에서는 순서가 역전됐었다(카지노 0.370 < 룸서비스 0.558) — 창의 문제였다.
  탭1 배너·통제 절, 탭3 캡션, `prescriptions()` 의
...
- ARS **1,247행** (2023-01~2026-06, 2025-11 제외) · `recv_total` 전 구간 유효 ·
  ARS/모바일 **접수자** 분해는 2024-12 31일만 · 날짜 중복 0
- 컴프 수량 445,472 (그중 `(V)` 접두만 30,560, `무료` 포함 414,912)

**⚠ 아래 3개는 창에 따라 답이 갈린다. 인용할 때 창을 반드시 함께 적는다.**

| 지표 | 구간 A (31일) | **구간 B (731일 · 기본)** |
|---|---|---|
| 원 상관 `recv_total↔casino` | 0.801 | **0.801** (불변) |
| 〃 roomservice / local | 0.740 / 0.380 | **0.765 / 0.409** |
| 요일 통제 편상관 casino | 0.370 (p 0.043) | **0.617 (p ≤ 0.001)** |
| 〃 room / local | 0.558 / 0.176 (p 0.344) | **0.596 / 0.214** (둘 다 p ≤ 0.001) |
| 특산품 래그 best_lag | **D+1** (0.467 > 0.370) | **D+0** (0.313 > 0.163) |

- **특산품 D+1 은 31일 창의 산물이었다.** 두 해가 각각 D+0 을 가리킨다
  (2023: 0.490 vs 0.299 · 2024: 0.422 vs 0.261). `test_local_goods_lag_is_same_day_on_the_full_window`
  가 **양쪽 창을 동시에** 고정한다 — 한쪽만 두면 "창이 답을 바꾼다"는 사실이 사라진다.
  다만 **요일 피크 어긋남은 731일에서도 참**이다(유입 토 1.38 / 특산품 일 1.36).
  체크아웃 서사는 이 피크 차이로 말하고 래그 상관으로 말하지 않는다.
- 편상관 순서 역전도 창의 문제였다 — 731일에서는 카지노가 1위를 되찾고 요일
  기여분이 54% → 23% 로 줄어든다. seed 고정(`S.CONFOUND_SEED`)이라 p 도 회귀값이다
- VTS: `corr(inflow, vrb)` 0.9988(= base 축을 빼는 근거) · base 제외 후
  `corr(inflow, vts)` 0.889(제외 전 0.981) · 상위 10일 중 유입 상위 밖 2일
- 요일 인덱스 특산품 일 1.629 / 목 0.642 (구간 A) · 2년 창은 토 1.20 / 일 1.36 —
  "특산품만 토요일에 죽는다"(구간 A 토 0.92)는 **재현되지 않는다**
...
- `buy_rate == tickets / winners` 전 1,247행 일치 (원본 정합성).
  ⚠ 2023-08 31일치만 소수 표기(0.41~0.53)로 들어오는데 `normalize_ars` 의
  "1 을 넘으면 100 으로 나눈다" 규칙이 그대로 흡수한다. **단위를 일괄 변환하도록
  바꾸지 말 것** — 그 31일이 100배로 튄다
- CSM 상위 20 위에 컴프 상품이 없을 것 · lift 는 유한하고 표본 미달은 NaN
- CSM 게이트: 31일 창 → 7일 / 카지노 129·특산품 40·룸서비스 16개 · 731일 창 →
  147일 / 983·356·87개 · 두 창 모두 lift 최댓값 < 10
- 탄력성 채움값(실측 중앙값): 31일 창 0.354 · 731일 창 0.285 ← 둘 다 0.5 미만이라는
  것이 "0.5 는 중립이 아니다"의 근거. 미측정 상품이 1위가 되지 않을 것
- 골프 2,703행 (원본 2,750 − 빈 패딩 47) · 판매 채널에 섞이지 않을 것

**구간 A(2024-12) 기반 수치는 데이터 확장 후에도 전부 그대로다.** 바뀐 것은
"어느 창을 기본으로 보느냐"이지 구간 A 의 값 자체가 아니다. 이 구분이 회귀
테스트를 읽는 열쇠다.

실데이터(`data/raw`)가 없으면 해당 테스트는 skip 된다. 현재 151개 전부 통과.
...
성공 기준을 먼저 정하고 끝까지 돌린다. 이 저장소의 기본 기준:
  ...
  1. `pytest tests\ -q` 전량 통과 — 151개. skip 은 `data/raw` 부재 시에만 정상
  2. 수치를 바꿨다면 <document_sync> 표의 대상 문서를 전부 갱신
  3. UI 를 바꿨다면 5개 탭을 전부 확인 — 탭 단위 예외 격리라 한 탭만 조용히 죽는다
