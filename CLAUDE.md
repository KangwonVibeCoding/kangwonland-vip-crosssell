...
| `demo` | Open API → `demographics_api.csv` | 2026-06-10 ~ 08-06 (일별) | 탭1 |
| `merchants` | Open API → `merchants_geocoded.csv` | — | 탭4 |

#### `demo` — 서버 다운이 아니라 파라미터 누락이었다 (2026-08-08 정정)

2026-08-04 에 이 API 를 "제공기관 서버 다운"으로 기록했으나 **오진이었다.**
`HTTP_ERROR(코드 04)` 는 서버 장애와 **필수 파라미터 누락을 구분하지 않는다.**
이 엔드포인트는 `dateFrom`/`dateTo` 가 필수인데 우리가 보내지 않았다.

```
날짜 없음                → 04 HTTP_ERROR              ← 나흘간 서버 탓으로 오해
dateFrom=20260601        → 33 DATE_STRING_PATTERN_MISSMATCH
dateFrom=2026-06-01      → resultCode 0 Success       ← 형식은 YYYY-MM-DD 고정
```

`pageNo`/`numOfRows` 도 함께 보내야 한다(날짜만 보내면 빈 응답). `type=json` 은
있으나 없으나 무관 — 응답은 항상 JSON 이다. 파라미터는 `S.DEMO_PARAMS`.
**04 를 만나면 서버를 탓하기 전에 포털 문서의 필수 요청변수부터 대조할 것.**

받아보니 데이터의 성격도 기록과 달랐다. 발표 서사에 직접 영향이 있다:

- **일자별이다** (`BSN_DT`). "기간 집계라 일별 분해 불가"는 더 이상 사실이 아니다.
- **그런데 보유분이 2026-06-10 ~ 08-06 (58일) 뿐이다.** 판매(2023~2024)와 겹치는
  날이 **0일**, 구간 A(2024-12)는 조회하면 `totalCount 0`. ARS 와는 21일
  (2026-06-10~06-30) 겹친다. 그래서 **VTS 의 base 축은 여전히 되살아나지 않는다** —
  `base_is_redundant()` 는 계속 발동한다. 일자별 데이터가 들어와도 그것이
  판매 기간과 겹쳐야 의미가 있다는 점이 `VTS_WEIGHTS` 주석의 숨은 전제였다.
...
- **`vip_ratio` 0.666 은 독립 출처에서 교차검증됐다.** 별도 공공데이터인
  하이원멤버십 연령별 성별 가입현황(회원 명부, 2024-12-31 기준 469,645명)의
  40~60대 비중이 **0.671**, 회원 정리 이후 안정 구간(2024-07~2026-05) 평균이
  **0.663** 이다. 방문객 집계와 회원 명부라는 **다른 모집단에서 같은 값**이 나온다.
  이 데이터셋 자체는 편입하지 않았다 — 월말 스냅샷(stock)이라 일별 분해가 안 되고,
  2024-03 에 회원 -34% 구조적 단절이 있으며, 12월마다 VIP 비중이 +0.015 오르는 것은
  계절성이 아니라 **연말 나이 일괄 갱신**이다(20대가 8~10천 명씩 위 밴드로 이동).
  숫자 하나를 얻자고 로더·스키마·폴백·테스트를 늘릴 이유가 없어 사실만 남긴다.
...
- **VRB / VTS** (`vip.py`) — VIP 모수와 타겟 지수. VTS 상위 10일 = 캠페인 집행 캘린더.
  ...
  ⚠ **base(VRB) 축은 유입 축의 복사본이라 자동으로 빠진다.** `vip_ratio` 가 상수 →
  `VRB = tickets × 상수`(실측 r=0.9988). 두 축을 다
  ...
  **headroom 으로 이관**한다(균등 재정규화하면 유입 실효 비중이 0.35→0.47 로 되레
  ...
  CII 의 `pressure_signal` 과 같은 규약이다. **판매 기간과 겹치는** 일자별 성별·연령이
  들어오면 자동 복구된다 — 2026-08-08 에 일자별 데이터를 받았지만 보유분이
  2026-06-10 이후뿐이라 겹치지 않아 여전히 상수다(위 `demo` 절 참고).
...
- 요일 인덱스 특산품 일 1.629 / 목 0.642 (구간 A 기준 — 데이터 교체 후에도 불변)
  ⚠ **구간 A 는 토요일이 4일뿐이다.** 같은 지수를 2년 창으로 재면 토 1.20 / 일 1.36
  이라 "특산품만 토요일에 죽는다"(구간 A 토 0.92)는 **재현되지 않는다.** 재현되는
  것은 일요일 > 토요일 방향뿐이다(2023·2024 각각 1.36 vs 1.20). D+1 근거를 인용할
  때 낙차(+77%)가 아니라 **재현성**으로 말한다 — 래그 상관 CI 가 이미 0 을 포함해서
  이 요일 프로파일이 유일한 보강 근거이기 때문이다
...
- 지도는 **pydeck** 이다. streamlit 이 이미 의존하므로 추가 패키지가 0개다.
  folium 은 `streamlit-folium` 별도 설치가 필요해 Cloud 빌드 리스크를 늘려서
  제외했다 — README 기술 스택에도 그렇게 적혀 있다(불일치 해소됨).
